...
//     let newTab = window.open("images/LALITHENDRA_RESUME.pdf", "_blank"); 
//     if (newTab) {
//         newTab.document.title = "Lalitendra Resume"; // Set title
//     }
// });
...
function initializeSkillBars() {
    ...
    skillBars.forEach(bar => {
        ...
        // Force a reflow
        bar.offsetWidth;
        // Add the animated-progress class
    ...
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            progressBars.forEach(bar => {
                ...
                bar.offsetWidth;                       // Force reflow for transition
                requestAnimationFrame(() => {
                    bar.style.width = bar.getAttribute('data-percentage') + '%';  // Animate to target %
                });
                bar.classList.add('animated-progress'); // Add animation CSS class
            });
        }
    });
}, { threshold: 0.5 });
